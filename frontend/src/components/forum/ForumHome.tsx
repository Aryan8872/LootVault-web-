  ...
  const fetchPosts = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3000/api/forum/posts', {
        params: {
          page: state.currentPage,  // Current page for pagination
          limit: state.limit,  // Limit per page
          search: state.search,  // Search query if applicable
          userData: userData,  // Ensure userData is passed if needed
        },
      });
