...
test.describe('ProductDetails Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:5200/game/67c1c655ef795cd0c9b16af8'); // Adjust ID dynamically if needed
  });
  ...
  test('should display product details correctly', async ({ page }) => {
    ...
    await expect(page.locator('h1.text-5xl')).toHaveText('game last of us');

    // Verify price (target span explicitly)
    await expect(page.locator('span.text-5xl')).toHaveText('2000');

    // Verify description (ensure we select only the <p> tag)
    const descriptionLocator = page.locator('p.text-gray-700');
    await expect(descriptionLocator).toHaveText('last of us wala with image');
